...
class MashahdScraper(BaseScraper, PlaywrightMixin):
    name = "mashahd"
    base_urls = [
        "https://mashahd.net",
        "https://mashahd.tv",
    ...
    def _parse_matches(self, html: str, base_url: str) -> List[Match]:
        ...
        containers = soup.select(
            ".match-item, .event-item, .live-item, "
            ".match-box, .card, article, .post, "
            "[class*=match], [class*=live]"
        )

        for container in containers:
            title_el = container.select_one(
                "h2, h3, h4, .title, .match-title, "
                ".event-title, a[title], .post-title"
            )
            ...
            league_el = container.select_one(
                ".league, .category, .tournament, .competition, [class*=league]"
            )
